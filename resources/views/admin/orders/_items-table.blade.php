...
@php
    ...
    $costoPorMillarGlobal = 1.00; // Default $1.00 MXN por millar
    ...
    <div class="card-body p-0">
        <table class="table mb-0" style="font-size: 15px;">
                        ...
                        <td style="padding: 10px 8px;">
                            ...
                            @if($requiresDesigns && $hasLinkedDesigns)
                                <div class="collapse mt-2" id="designDetails{{ $item->id }}">
                                    <div style="background: #fafafa; border-radius: 6px; padding: 8px; border: 1px solid #e0e0e0;">
                                        @foreach($linkedDesigns as $design)
                                            <div class="d-flex align-items-center justify-content-between {{ !$loop->last ? 'mb-2 pb-2' : '' }}" style="{{ !$loop->last ? 'border-bottom: 1px solid #e0e0e0;' : '' }}">
                                                <div class="d-flex align-items-center">
                                                    @if($design->svg_content)
                                                        <div style="width: 28px; height: 28px; background: white; border-radius: 4px; display: flex; align-items: center; justify-content: center; overflow: hidden; margin-right: 8px; border: 1px solid #e0e0e0;">
                                                            {!! $design->svg_content !!}
                                                        </div>
                                                    @else
                                                        <div style="width: 28px; height: 28px; background: #1565c0; border-radius: 4px; display: flex; align-items: center; justify-content: center; margin-right: 8px;">
                                                            <i class="fas fa-vector-square" style="color: white; font-size: 12px;"></i>
                                                        </div>
                                                    @endif
                                                    <div>
            ...
            <tfoot style="background: #f8f9fa;">
                <tr>
                    <td colspan="4" class="text-right" style="font-size: 14px; color: #212529;"><strong>Subtotal:</strong></td>
                    <td class="text-right" style="font-size: 15px; color: #212529;"><strong>${{ number_format($order->subtotal, 2) }}</strong></td>
                </tr>
                @if ($order->discount > 0)
                    <tr>
                        <td colspan="4" class="text-right" style="font-size: 14px; color: #c62828;">Descuento:</td>
                        <td class="text-right" style="font-size: 15px; color: #c62828;">-${{ number_format($order->discount, 2) }}</td>
                    </tr>
                @endif
                @if ($order->requires_invoice && $order->iva_amount > 0)
                    <tr>
                        <td colspan="4" class="text-right" style="font-size: 14px; color: #212529;">IVA 16%:</td>
                        <td class="text-right" style="font-size: 15px; color: #212529;">${{ number_format($order->iva_amount, 2) }}</td>
                    </tr>
                @endif
                <tr style="background: #007bff;">
                    <td colspan="4" class="text-right" style="color: white;"><strong style="font-size: 16px;">TOTAL:</strong></td>
                    <td class="text-right" style="color: white;"><strong style="font-size: 20px;">${{ number_format($order->total, 2) }}</strong></td>
                </tr>
            </tfoot>
        </table>
    </div>
                    ...
                    <div class="modal-header" style="background: #6f42c1; color: white;">
                        <h5 class="modal-title">
                            ...
                            @if ($item->variant?->attributes_display)
                                <span class="badge badge-light ml-2" style="font-size: 12px;">{{ $item->variant->attributes_display }}</span>
                            @endif
                        </h5>
                                ...
                                @if(!empty($value) && $value !== '0')
                                    ...
                                    <div class="col-md-4 col-6 mb-3">
                                        <div class="medida-card-show text-center">
                                            @if($imageName)
                                                <img src="{{ asset('images/' . $imageName) }}" alt="{{ $labelText }}" class="medida-img-show">
                                            @endif
        ...
        <div class="modal-content">
            <div class="modal-header" style="background: linear-gradient(135deg, #1565c0 0%, #0d47a1 100%); color: white;">
                <h5 class="modal-title" id="selectDesignForItemModalTitle">
                    <i class="fas fa-link mr-2"></i> Vincular Diseño Técnico
                </h5>
                <button type="button" class="close text-white" data-dismiss="modal">&times;</button>
            </div>
            <div class="modal-body">
                    ...
                    <div class="col-12 col-md-8 mb-2 mb-md-0">
                        <div class="input-group">
                            <div class="input-group-prepend">
                                <span class="input-group-text" style="background: #e3f2fd; border-color: #90caf9;">
                                    <i class="fas fa-search" style="color: #1565c0;"></i>
                                </span>
                            </div>
                            <input type="text" class="form-control" id="searchDesignForItem" placeholder="Buscar diseño..." autocomplete="off">
                        </div>
                    </div>
                    <div class="col-12 col-md-4">
                        <button type="button" class="btn btn-block" id="btnSearchDesignForItem" style="background: #1565c0; color: white;">
                            <i class="fas fa-search mr-1"></i> Buscar
                        </button>
                    </div>
                </div>
                <div id="designsForItemResults" class="list-group" style="max-height: 350px; overflow-y: auto;"></div>
                <div id="designsForItemLoader" class="text-center p-4 d-none">
                    <div class="spinner-border text-primary"></div>
                    <p class="mt-2" style="color: #1565c0;">Buscando diseños...</p>
                </div>
                <div id="designsForItemEmpty" class="text-center p-4 d-none" style="background: #fff3e0; border-radius: 8px;">
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body text-center">
                <div id="previewDesignImage" style="min-height: 200px; background: #f5f5f5; border-radius: 8px; display: flex; align-items: center; justify-content: center; margin-bottom: 15px;">
...
(function() {
    ...
    window.openDesignSelectorForItem = function(itemId, itemName) {
        ...
        document.getElementById('selectDesignForItemModalTitle').innerHTML =
            '<i class="fas fa-link mr-2"></i> Vincular Diseño a: ' + escapeHtml(itemName);
    ...
    function filterAndRenderDesignsForItem() {
        ...
        if (query) {
            filtered = allDesignsForItem.filter(function(item) {
                var text = [item.export_name || '', item.design_name || '', item.app_type_name || ''].join(' ').toLowerCase();
    ...
    function renderDesignsForItem(items) {
        ...
        items.forEach(function(item) {
            var preview = item.svg_content
                ? '<div style="width:40px;height:40px;background:white;border-radius:4px;display:flex;align-items:center;justify-content:center;overflow:hidden;">' + item.svg_content + '</div>'
            ...
            var html = '<div class="list-group-item d-flex justify-content-between align-items-center" style="cursor:pointer;" onclick="linkDesignToItemConfirm(' + item.id + ', \'' + escapeHtml(name).replace(/'/g, "\\'") + '\')">' +
                '<div class="d-flex align-items-center">' +
                    ...
                    '</div>' +
                '</div>' +
                '<button type="button" class="btn btn-sm" style="background:#1565c0;color:white;" onclick="event.stopPropagation();linkDesignToItemConfirm(' + item.id + ', \'' + escapeHtml(name).replace(/'/g, "\\'") + '\')">' +
    ...
    window.linkDesignToItemConfirm = function(designExportId, designName) {
        ...
        Swal.fire({
            ...
            html: '<p>Se vinculará <strong>' + escapeHtml(designName) + '</strong> al producto <strong>' + escapeHtml(itemName) + '</strong></p>',
            icon: 'question',
    ...
    function linkDesignToItemExecute(itemId, designExportId, designName) {
        Swal.fire({ title: 'Vinculando...', allowOutsideClick: false, showConfirmButton: false, didOpen: function() { Swal.showLoading(); } });
        fetch('/admin/orders/' + orderId + '/items/' + itemId + '/link-design', {
            ...
            body: JSON.stringify({ design_export_id: designExportId })
        })
        .then(function(r) { return r.json(); })
        .then(function(data) {
            if (data.success) {
                Swal.fire({ title: '¡Vinculado!', text: data.message, icon: 'success', timer: 2000, timerProgressBar: true })
                .then(function() { $('#selectDesignForItemModal').modal('hide'); window.location.reload(); });
            } else {
    ...
    window.unlinkDesignFromItem = function(orderId, itemId, designExportId, designName) {
        Swal.fire({
            ...
            html: '<p>Se desvinculará <strong>' + escapeHtml(designName) + '</strong> del producto</p>',
            icon: 'warning',
            showCancelButton: true,
            ...
            if (result.isConfirmed) {
                Swal.fire({ title: 'Desvinculando...', allowOutsideClick: false, showConfirmButton: false, didOpen: function() { Swal.showLoading(); } });
                ...
                .then(function(data) {
                    if (data.success) {
                        Swal.fire({ title: 'Desvinculado', icon: 'success', timer: 1500 }).then(function() { window.location.reload(); });
                    } else {
                        Swal.fire('Error', data.message, 'error');
                    }
                })
                .catch(function() { Swal.fire('Error', 'No se pudo conectar', 'error'); });
            }
        });
    };
            ...
            if (data.success) {
                ...
                if (design) {
                    ...
                    var imgHtml = design.svg_content
                        ...
                        : (design.image_url
                            ? '<img src="' + design.image_url + '" style="max-width:100%;max-height:300px;">'
                    ...
                    if (design.stitches_formatted) details += '<p><strong>Puntadas:</strong> ' + design.stitches_formatted + '</p>';
                    if (design.dimensions) details += '<p><strong>Dimensiones:</strong> ' + design.dimensions + '</p>';
                    if (design.file_format) details += '<p><strong>Formato:</strong> ' + design.file_format.toUpperCase() + '</p>';
                    if (design.application_type) details += '<p><strong>Aplicación:</strong> ' + design.application_type + '</p>';
                    details += '</div>';
