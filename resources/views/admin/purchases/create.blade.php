                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (!data.units || data.units.length === 0) {
                            $selectUnit.html('<option value="">Sin presentaciones</option>');
                            Swal.fire({
                                icon: 'warning',
                                title: 'Sin presentaciones configuradas',
                                text: 'Este material no tiene presentaciones de compra. Configure las conversiones del material primero.',
                        ...
                        data.units.forEach(function(unit) {
                            let displayName = '';
                            if (unit.is_base) {
                                displayName = `${unit.name} (${unit.symbol}) (Base)`;
                            } else {
                                // Mostrar: NOMBRE (símbolo) - factor de conversión
                                // Si tiene label, usarlo como referencia adicional
                                const labelPart = unit.label ? ` [${unit.label}]` : '';
                                displayName =
                                    `${unit.name}${labelPart} - ${unit.conversion_display}`;
                            }
                            options +=
                                `<option value="${unit.id}" data-factor="${unit.conversion_factor}" data-symbol="${unit.symbol}" data-name="${unit.name}">${displayName}</option>`;
                        });
