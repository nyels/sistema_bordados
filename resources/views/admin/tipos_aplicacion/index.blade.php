                    ...
                    <table id="example1" class="table table-bordered table-hover table-condensed">
                        ...
                        <tbody>
                            @foreach ($aplication_types as $aplication_type)
                                <tr style="text-align: center;" data-id="{{ $aplication_type->id }}">
                                    <td>{{ $loop->iteration }}</td>
                                    <td class="nombre-aplicacion">{{ $aplication_type->nombre_aplicacion }}</td>
                                    <td style="text-align: center;">
                                        <button type="button" class="btn btn-warning btn-edit"
                                            data-id="{{ $aplication_type->id }}"
                                            data-nombre="{{ $aplication_type->nombre_aplicacion }}">
            ...
            <div class="modal-content">
                <div class="modal-header bg-primary">
                    <h5 class="modal-title text-white" style="font-weight: bold;">
                        <i class="fas fa-plus-circle mr-2"></i> NUEVO TIPO DE APLICACION
                    </h5>
                ...
                <form id="formCreate">
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control form-control-sm" id="create_nombre_aplicacion"
                                name="nombre_aplicacion" placeholder="Ej: BORDADO"
            ...
            <div class="modal-content">
                <div class="modal-header bg-warning">
                    <h5 class="modal-title" style="font-weight: bold;">
                        <i class="fas fa-edit mr-2"></i> EDITAR TIPO DE APLICACION
                    </h5>
                    ...
                    <div class="modal-body">
                        ...
                        <div style="border-bottom: 3px solid #ffc107; padding-bottom: 8px; margin-bottom: 20px;">
                            <h5 style="color: #856404; font-weight: 600; margin: 0;">
                                <i class="fas fa-layer-group mr-2"></i>Datos del Tipo
                            </h5>
                        </div>
                        <div class="form-group">
                            <label>Nombre <span style="color: red;">*</span></label>
                            <input type="text" class="form-control form-control-sm" id="edit_nombre_aplicacion"
                                name="nombre_aplicacion" placeholder="Ej: BORDADO"
                                pattern="[a-zA-ZñÑáéíóúÁÉÍÓÚ\s]+" title="Solo se permiten letras y espacios"
                                oninput="this.value = this.value.replace(/[^a-zA-ZñÑáéíóúÁÉÍÓÚ\s]/g, '')" required>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header bg-danger text-white">
                    <h5 class="modal-title" style="font-weight: bold;">
                        <i class="fas fa-trash mr-2"></i> ELIMINAR TIPO DE APLICACION
                    </h5>
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label>Nombre</label>
                        <input type="text" class="form-control form-control-sm" id="delete_nombre_aplicacion" disabled>
                    </div>
                    <div class="alert alert-warning text-center mt-3">
                        <i class="fas fa-exclamation-triangle fa-2x mb-2"></i>
                        <p class="mb-0" style="font-weight: bold; font-size: 16px;">¿Deseas eliminar este tipo?</p>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">
    ...
    <script>
        $(function() {
            var table = $("#example1").DataTable({
                "pageLength": 10,
                "language": {
                    "emptyTable": "No hay informacion", "info": "Mostrando _START_ a _END_ de _TOTAL_ Tipos",
                    "infoEmpty": "Mostrando 0 a 0 de 0 Tipos", "infoFiltered": "(Filtrado de _MAX_ total Tipos)",
                    "lengthMenu": "Mostrar _MENU_ Tipos", "loadingRecords": "Cargando...", "processing": "Procesando...",
            ...
            $('#btnNuevo').on('click', function() {
                $('#create_nombre_aplicacion').val('').removeClass('is-invalid');
            ...
            $('#formCreate').on('submit', function(e) {
                e.preventDefault();
                var nombre = $('#create_nombre_aplicacion').val().trim();
                if (!nombre) {
                    $('#create_nombre_aplicacion').addClass('is-invalid');
                ...
                $.ajax({
                    url: '{{ route("admin.tipos_aplicacion.store") }}',
                    type: 'POST',
                    data: { _token: '{{ csrf_token() }}', nombre_aplicacion: nombre },
                    ...
                    error: function(xhr) {
                        if (xhr.status === 422) {
                            ...
                            if (errors && errors.nombre_aplicacion) {
                                $('#create_nombre_aplicacion').addClass('is-invalid');
                                $('#create_error_nombre').text(errors.nombre_aplicacion[0]);
            ...
            $(document).on('click', '.btn-edit', function() {
                ...
                $('#edit_nombre_aplicacion').val($(this).data('nombre')).removeClass('is-invalid');
            ...
            $('#formEdit').on('submit', function(e) {
                e.preventDefault();
                var id = $('#edit_id').val(), nombre = $('#edit_nombre_aplicacion').val().trim();
                if (!nombre) { $('#edit_nombre_aplicacion').addClass('is-invalid'); $('#edit_error_nombre').text('El nombre es obligatorio'); return; }
                $('#btnSaveEdit').prop('disabled', true).html('<i class="fas fa-spinner fa-spin"></i> Guardando...');
                $.ajax({
                    url: '/tipos_aplicacion/actualizar/' + id, type: 'PUT',
                    data: { _token: '{{ csrf_token() }}', nombre_aplicacion: nombre },
                    success: function(response) {
                        $('#modalEdit').modal('hide');
                        if (response.success) {
                            var row = $('tr[data-id="' + id + '"]');
                            row.find('.nombre-aplicacion').text(response.data.nombre_aplicacion);
                            row.find('.btn-edit').data('nombre', response.data.nombre_aplicacion);
                            row.find('.btn-delete').data('nombre', response.data.nombre_aplicacion);
                    ...
                    error: function(xhr) {
                        if (xhr.status === 422 && xhr.responseJSON.errors?.nombre_aplicacion) {
                            $('#edit_nombre_aplicacion').addClass('is-invalid');
                            $('#edit_error_nombre').text(xhr.responseJSON.errors.nombre_aplicacion[0]);
                        } else {
                            $('#modalEdit').modal('hide');
            ...
            $(document).on('click', '.btn-delete', function() {
                ...
                $('#delete_nombre_aplicacion').val($(this).data('nombre'));
            ...
            $('#btnConfirmDelete').on('click', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            url: '/tipos_aplicacion/eliminar/' + id, type: 'DELETE', data: { _token: '{{ csrf_token() }}' },
                            success: function(response) {
                                if (response.success) {
